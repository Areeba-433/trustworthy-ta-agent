...
class TokenService:
    ...
    @staticmethod
    def createSession(db: Session, userId: int, rememberMe: bool, ip: str, userAgent: str) -> dict:
        ...
        accessToken  = createAccessToken({"sub": str(userId)}, accessExp)
        refreshToken = createRefreshToken({"sub": str(userId)}, refreshExp)
    ...
    def refreshAccessToken(db: Session, refreshToken: str):
        ...
        newToken = createAccessToken({"sub": payload["sub"]})
